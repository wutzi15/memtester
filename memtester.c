...
void check_posix_system(void) {
  if (sysconf(_SC_VERSION) < 198808L) {
    fprintf(stderr,
            "A POSIX system is required.  Don't be surprised if "
            "this craps out.\n");
...
void usage(char *me) {
  fprintf(stderr,
          "\n"
          "Usage: %s [-p physaddrbase [-d device] [-u] [-q]] <mem>[B|K|M|G] "
          "[loops]\n",
          me);
  exit(EXIT_FAIL_NONSTARTER);
}
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "qp:d:u")) != -1) {
    switch (opt) {
      case 'q':
        quick_test = 1;
        if (!testmask) {
          /* right-to-left: (Stuck-is-always-on) and Rnd MUST be on
           * Rnd,XOR,SUB,MUL DIV,OR,AND,Inc Bit,Seq,Checker,Spread
           * Flip,Ones,Zeros[,8bit 16bit] */
          testmask = 0xE7;
  ...
  if (!do_mlock)
    fprintf(stderr,
            "Continuing with unlocked memory; testing "
